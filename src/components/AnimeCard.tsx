...
export default function AnimeCard({ anime, index = 0, variant = "default" }: AnimeCardProps) {
      ...
      <Link to={`/anime/${anime.animeId}`}>
        ...
        >
          ...
          <div className="flex-1 min-w-0 py-0.5">
            <h4 className="text-sm text-gray-200 font-medium line-clamp-2 leading-tight">{anime.title}</h4>
            <div className="flex items-center gap-2 mt-1.5">
              <span className="text-[10px] px-1.5 py-0.5 bg-purple-500/20 text-purple-300 rounded">{anime.status || "Ongoing"}</span>
              {anime.score && (
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.04 }}>
          <div className="relative aspect-[3/4] rounded-lg overflow-hidden bg-gray-800 group">
            ...
            {anime.episodes !== undefined && (
              <span className="absolute top-1.5 left-1.5 text-[10px] px-1.5 py-0.5 bg-purple-600 text-white rounded font-medium">EP {anime.episodes}</span>
            )}
          </div>
          <h3 className="mt-1.5 text-xs text-gray-300 line-clamp-2 leading-tight font-medium">{anime.title}</h3>
        </motion.div>
      </Link>
    );
  }

  // Grid variant - optimized for 3-column layout
  if (variant === "grid") {
    return (
      <Link to={`/anime/${anime.animeId}`} className="block">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }}>
          <div className="relative aspect-[3/4] rounded-lg overflow-hidden bg-gray-800 group shadow-md">
  ...
  // Default variant - for horizontal scroll
  return (
    <Link to={`/anime/${anime.animeId}`} className="block w-[140px] sm:w-[160px] flex-shrink-0">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
        <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-gray-800 group shadow-lg">
          ...
          {anime.episodes !== undefined && (
            <span className="absolute top-2 left-2 text-xs px-2 py-0.5 bg-purple-600 text-white rounded-md font-medium">EP {anime.episodes}</span>
          )}
          {anime.releaseDay && (
            <span className="absolute bottom-2 left-2 text-[10px] px-1.5 py-0.5 bg-white/20 backdrop-blur-sm text-white rounded">{anime.releaseDay}</span>
          )}
        </div>
        <h3 className="mt-2 text-sm text-gray-200 line-clamp-2 leading-tight font-medium group-hover:text-purple-400 transition-colors">{anime.title}</h3>
        {anime.score && (
